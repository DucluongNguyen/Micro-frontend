...
Xem mục "Adding another remote" trong [`container/README.md`](container/README.md)
— tóm tắt: thêm remote vào `REMOTE_DEFINITIONS`
(`module-federation.config.ts`), thêm biến `URL_HOST_<NAME>`, khai báo type
trong `src/types/remotes.d.ts`, thêm route `/<name>/*`, và thêm entry vào
`REMOTES` trong `useRemoteNavigation.ts` để hiện trên sidebar.
